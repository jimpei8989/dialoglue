...
def evaluate(args, model, tokenizer, processor, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        results = eval_metric(
            model, inputs, outputs[0], outputs[1], outputs[2], outputs[3], outputs[4], outputs[5]
        )
...
def predict_and_format(
    ...
    for i in range(len(ids)):
        ...
        for slot in model.slot_list:
            ...
            elif "true" in model.class_types and class_prediction == model.class_types.index(
                "true"
            ):
                dialog_state[slot] = "true"
            elif "false" in model.class_types and class_prediction == model.class_types.index(
                "false"
            ):
        ...
        for slot in model.slot_list:
            ...
            if "refer" in model.class_types and class_prediction == model.class_types.index(
                "refer"
            ):
...
def main():
    ...
    parser.add_argument(
        "--few_shot", action="store_true", help="Whether to run few-shot training."
    )
    ...
    parser.add_argument(
        "--do_eval", action="store_true", help="Whether to run eval on the <predict_type> set."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dropout_rate", default=0.3, type=float, help="Dropout rate for BERT representations."
    )
    parser.add_argument(
        "--heads_dropout", default=0.0, type=float, help="Dropout rate for classification heads."
    )
    parser.add_argument(
        "--class_loss_ratio",
        default=0.8,
    ...
    parser.add_argument(
        "--mlm_during", action="store_true", help="Do MLM multi-task training on the dataset."
    )
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(
            args, train_dataset, features, model, tokenizer, processor, continue_from_global_step
        )
